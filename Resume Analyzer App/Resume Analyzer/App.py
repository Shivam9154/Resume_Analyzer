...
def run():
    ...
    if choice == 'Normal User':
        ...
        if st.button('Login'):
            if ad_user == 'Shivam Patil' and ad_password == 'shivam123':
                st.success("Welcome Shivam")
                # Display Data
                cursor.execute('''SELECT*FROM user_data''')
                data = cursor.fetchall()
                st.header("**User's👨‍💻 Data**")
                df = pd.DataFrame(data, columns=['ID', 'Name', 'Email', 'Resume Score', 'Timestamp', 'Total_Page',
                                                 'Predicted_Field', 'User_Level', 'Actual_Skills', 'Recommended_Skills',
                                                 'Recommended_Course'])
                st.dataframe(df)
                st.markdown(get_table_download_link(df,'User_Data.csv','Download Report'), unsafe_allow_html=True)
                st.header("Data Scientist Profile Candidates")
                df1=df[df["Predicted_Field"]=="Data Science"]
                st.dataframe(df1)
                st.header("Web Developer Profile Candidates")
                df1=df[df["Predicted_Field"]=="Web Development"]
                st.dataframe(df1)
                st.header("UI-UX Developer Profile Candidates")
                df1=df[df["Predicted_Field"]=="UI-UX Development"]
                st.dataframe(df1)
                ## Admin Side Data
